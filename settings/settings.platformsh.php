...
if ($platformsh->hasRelationship('redis') && !InstallerKernel::installationAttempted() && extension_loaded('redis') && class_exists('Drupal\redis\ClientFactory')) {
  ...
  // Manually add the classloader path, this is required for the
  // container cache bin definition below
...
// The 'trusted_hosts_pattern' setting allows an admin to restrict
// the Host header values that are considered trusted.
// If an attacker sends a request with a custom-crafted Host header
// then it can be an injection vector, depending on how the Host header is used.
// However, Platform.sh already replaces the Host header with the route
// that was used to reach Platform.sh, so it is guaranteed to be safe.
...
foreach ($platformsh->variables() as $name => $value) {
  ...
  switch ($prefix) {
    ...
    case 'drupal':
      ...
      // `foo` becomes $config['conf_file']['prop']['subprop'] = 'foo';
      // Example: Variable `d8config:conf_file:prop:subprop` with value
      // ['foo' => 'bar'] becomes
